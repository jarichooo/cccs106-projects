...
def main(page: ft.Page):
    ...
    def generate_profile(e):
        try:
            ...
            # Generate profile
            def black_text(value, size=16, weight=None):
                return ft.Text(value, size=size, weight=weight, color=ft.Colors.BLACK)

            profile_content = ft.Column([
                black_text("🎓 STUDENT PROFILE", size=20, weight=ft.FontWeight.BOLD),
                ft.Divider(),
                black_text(f"👤 Full Name: {first_name.value} {last_name.value}"),
                black_text(f"🆔 Student ID: {student_id.value or 'Not provided'}"),
                black_text(f"🎂 Age: {age.value} years old"),
                black_text(f"📅 Birth Year: {birth_year}"),
                black_text(f"📚 Program: {program_dropdown.value or 'Not selected'}"),
                black_text(f"📊 Year Level: {year_level.value or 'Not selected'}"),
                black_text(f"🎨 Favorite Color: {favorite_color.value or 'Not selected'}"),
                black_text(f"🎯 Hobbies: {hobbies.value or 'Not provided'}"),
                ft.Divider(),
                black_text(f"🎓 Expected Graduation: {graduation_year}", weight=ft.FontWeight.BOLD),
                black_text(f"📝 Profile generated on: {datetime.now().strftime('%B %d, %Y at %I:%M %p')}", size=12),
            ])
